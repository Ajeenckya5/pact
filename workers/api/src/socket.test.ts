...
import { AUTH_MS, beginSocket, consumeSocketAuth, type ReplayCache } from "./socket";
...
describe("socket challenge", () => {
  ...
  it("closes the challenge after 5 seconds", async () => {
    const keys = await createSigningKey();
    const state = beginSocket(1_000);
    const raw = JSON.stringify({ pk: keys.pk, sig: await signBytes(keys.sk, socketCanon(state.nonce, "p1", keys.pk)) });
    assert.equal(await consumeSocketAuth(state, raw, memoryReplay(), 1_000 + AUTH_MS + 1, "p1"), "timeout");
  });

  it("allows the site origins and rejects a signature carried on the URL", () => {
